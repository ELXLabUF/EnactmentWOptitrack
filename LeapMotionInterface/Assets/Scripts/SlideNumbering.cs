...
public class SlideNumbering : MonoBehaviour
{
    ...
    private bool leapCaptureOn = true;
    ...
    void Start()
    {
        ...
        switch (sceneNotesOn)
        {
            case true:
                break;
            case false:
                Destroy(GameObject.Find("WriteSlide"));
                break;
            default:
                break;
        }
    ...
    {   
        ...
        GameObject.FindGameObjectWithTag("all_canvases").GetComponent<CanvasManager>().planningHide();
    }
    ...
    public void playSelectedSlide()
    {
        ...
        /* if(leapCaptureOn && getSelectedData().getPlaying() == false)
         {
             StartCoroutine(replayLeap(getSelectedSection(), getSelectedSlideIndex()));
         }
         else if (leapCaptureOn && getSelectedData().getPlaying() == true)
         {
             getSelectedData().stopAudio();
         }
         else
         {
             getSelectedData().playAudio();
         }*/
        //else { StopAllCoroutines(); getSelectedData().setPlaying(false); }
        if (leapCaptureOn)
        {
            if (getSelectedData().getPlaying() == false)
            {
                StartCoroutine(replayLeap(getSelectedSection(), getSelectedSlideIndex()));
            }
            else
    ...
    {
        getSelectedData().updateEnactmentScreen(0);
    ...
    public void updateNewEnactmentObjects(int sect, int id)
    {
        ...
        for (int i = 0; i < children.Length; i++)
        {
            ...
            for (int k = 0; k < grandchildren.Length; k++)
            {
                ...
                {
                    grandchildrenData[k].updateEnactmentScreen(0); 
 ...
 /*   public void handheldObjects()
    ...
    */
    ...
    public bool getSceneNotes()
    {
        return sceneNotesOn;
    }

    public bool getLeapMotion()
    {
        return leapCaptureOn;
    }
    ...
    private int getSelectedSlideIndex()
    {
        SlideArray[] children = GetComponentsInChildren<SlideArray>();
    ...
    public IEnumerator playLeap(int whichSect, int whichSlide)
    {
        SlideArray[] sections = GetComponentsInChildren<SlideArray>();

        SlideData[] section = sections[whichSect].GetComponentsInChildren<SlideData>();
        SlideData slide = section[whichSlide];

        Vector4[] charaAnim = slide.getCharaLeapAnim();
        //Vector4[] objAnim = slide.getObjLeapAnim();
        for(int i = 0; i < charaAnim.Length; i++)
        {
            slide.updatePlayScreen(i);
            yield return new WaitForSeconds(charaAnim[i].w);
        }
        
    }

    public IEnumerator replayLeap(int whichSect, int whichSlide)
    {
        SlideArray[] sections = GetComponentsInChildren<SlideArray>();
    ...
    public IEnumerator SequenceTiming(bool saveFiles)
      {
        GameObject.FindGameObjectWithTag("all_canvases").GetComponent<CanvasManager>().saveStoryHide();
        ...
        if (saveFiles)
        {
            if (System.IO.File.Exists(participantName + "/"+fileNum + "_" + "mainTitle.png"))
            { ScreenCapture.CaptureScreenshot(participantName + "/" + fileNum + "_" + "mainTitle2.png"); }
            else { ScreenCapture.CaptureScreenshot(participantName + "/" + fileNum + "_" + "mainTitle.png"); }
        ...
        if (saveFiles)
        {
            if (System.IO.File.Exists(participantName + "/" + fileNum + "_" + "begTitle.png"))
            { ScreenCapture.CaptureScreenshot(participantName + "/" + fileNum + "_" + "begTitle2.png"); }
            else { ScreenCapture.CaptureScreenshot(participantName + "/" + fileNum + "_" + "begTitle.png"); }
        ...
        for (int i = 0; i < beginning.Length; i++)
          {
            ...
            if(!leapCaptureOn)
            {
                beginning[i].updatePlayScreen(-1);
            }
            ...
            {
                string framename;
                if (System.IO.File.Exists(participantName + "/"+fileNum + "_" + "begFrame_" + x + ".png")||doubleTroubleFrame==true)
                {
                    framename = participantName + "/" + fileNum + "_" + "begFrame2_" + x + ".png";
                    doubleTroubleFrame = true;
                }
                else
                {
                    framename = participantName + "/" + fileNum + "_" + "begFrame_" + x + ".png";
                }
            ...
            if (leapCaptureOn)
            {
                StartCoroutine(playLeap(0, i));
            }
            yield return new WaitForSeconds(wait);
            StopCoroutine(playLeap(0,i));

            if (saveFiles)
            {
                
                    if (System.IO.File.Exists(participantName +"/"+fileNum + "_" + "begAud_" + x)||doubleTroubleAud==true)
                    {
                        GetComponent<SavWav>().Save(participantName + "/" + fileNum + "_" + "begAud2_" + x, beginning[i].getAudio());
                    doubleTroubleAud = true;
                        //whichClip = 2;
                    }
                    else
                    {
                        GetComponent<SavWav>().Save(participantName + "/" + fileNum + "_" + "begAud_" + x, beginning[i].getAudio());
                       // whichClip = 2;
                    }
        ...
        if (saveFiles)
        {
            if (System.IO.File.Exists(participantName + "/"+fileNum + "_" + "midTitle.png"))
            { ScreenCapture.CaptureScreenshot(participantName + "/" + fileNum + "_" + "midTitle2.png"); }
            else { ScreenCapture.CaptureScreenshot(participantName + "/" + fileNum + "_" + "midTitle.png"); }
        ...
        for (int i = 0; i < middle.Length; i++)
          {
            ...
            {
                string framename;
                if (System.IO.File.Exists(participantName + "/"+ fileNum + "_" + "midFrame_" + x + ".png")||doubleTroubleFrame==true)
                {
                    framename = participantName + "/" + fileNum + "_" + "midFrame2_" + x + ".png";
                    doubleTroubleFrame = true;
                }
                else
                {
                    framename = participantName + "/" + fileNum + "_" + "midFrame_" + x + ".png";
                }
                ScreenCapture.CaptureScreenshot(framename);
            }
            ...
            if (leapCaptureOn)
            {
                StartCoroutine(playLeap(1, i));
            }
            yield return new WaitForSeconds(wait);
            StopCoroutine(playLeap(1, i));

            if (saveFiles)
            {
                
                    if (System.IO.File.Exists(participantName + "/"+ fileNum + "_" + "midAud_" + x)||doubleTroubleAud==true)
                    {
                        GetComponent<SavWav>().Save(participantName + "/"+ fileNum + "_" + "midAud2_" + x, middle[i].getAudio());
                    //whichClip = 2;
                    doubleTroubleAud = true;
                    }
                    else
                    {
                        GetComponent<SavWav>().Save(participantName + "/"+ fileNum + "_" + "midAud_" + x, middle[i].getAudio());
                        //whichClip = 2;
                    }
        ...
        if (saveFiles)
        {
            if (System.IO.File.Exists(participantName + "/"+ fileNum + "_" + "endTitle.png"))
            { ScreenCapture.CaptureScreenshot(participantName + "/"+ fileNum + "_" + "endTitle2.png"); }
            else { ScreenCapture.CaptureScreenshot(participantName + "/"+ fileNum + "_" + "endTitle.png"); }
        ...
        for (int i = 0; i < end.Length; i++)
          {
            ...
            {
                string framename;
                if (System.IO.File.Exists(participantName + "/"+ fileNum + "_" + "endFrame_" + x + ".png") || doubleTroubleFrame == true)
                {
                    framename = participantName + "/"+ fileNum + "_" + "endFrame2_" + x + ".png";
                    doubleTroubleFrame = true;
                }
                else
                {
                    framename = participantName + "/"+ fileNum + "_" + "endFrame_" + x + ".png";
                }
                ScreenCapture.CaptureScreenshot(framename);
            }
            ...
            if (saveFiles)
            {
                    ...
                    if (System.IO.File.Exists(participantName + "/"+ fileNum + "_" + "endAud_" + x)||doubleTroubleAud==true)
                    {
                        GetComponent<SavWav>().Save(participantName + "/"+ fileNum + "_" + "endAud2_" + x, end[i].getAudio());
                    //whichClip = 2;
                    doubleTroubleAud = true;
                    }
                    else
                    {
                        GetComponent<SavWav>().Save(participantName + "/"+ fileNum + "_" + "endAud_" + x, end[i].getAudio());
                        //whichClip = 2;
                    }
        ...
        if (saveFiles)
        {
            if (System.IO.File.Exists(participantName + "/"+fileNum + "_" + "theEnd.png"))
            { ScreenCapture.CaptureScreenshot(participantName + "/"+ fileNum + "_" + "theEnd2.png"); }
            else { ScreenCapture.CaptureScreenshot( participantName + "/"+ fileNum + "_" + "theEnd.png"); }
        ...
        if (saveFiles)
        {
            GameObject.Find("CanvasManager").GetComponent<CanvasManager>().toEnd();
        }
        fileNum += 1;
        if (saveFiles)
        {
            /*StreamWriter writer;
            if (System.IO.File.Exists(participantName + "/" + fileNum + "_" + "sceneNotes.txt"))
            {
                 writer = new StreamWriter(participantName + "/"+ fileNum + "_" + "sceneNotes2.txt", true);
            }
            else
            {
                 writer = new StreamWriter(participantName + "/" + fileNum + "_" + "sceneNotes.txt", true);
            }*/
          
                var file = File.Create(participantName + "/" + fileNum + "_" + "sceneNotes.txt");
            var writer = new StreamWriter(file, System.Text.Encoding.ASCII);
                ...
                writer.WriteLine(beginning[i].getSceneInfo());
                ...
                writer.WriteLine(middle[i].getSceneInfo());
                ...
                writer.WriteLine(end[i].getSceneInfo());
    ...
    public void recordButton()
    {
        switch (condition)
        {
            case 0:
                ...
                {
                    GameObject.FindGameObjectWithTag("all_canvases").GetComponent<CanvasManager>().toEnactment();
                    updateEnactmentObjects();
                    updateEnactmentPose();
                }
                else { GameObject.FindGameObjectWithTag("all_canvases").GetComponent<CanvasManager>().recordScenePop(); }
    ...
    public void planningButton()
    {
        switch (condition)
        {
            ...
            case 2:
                if (planningReady)
                {
                    GameObject.FindGameObjectWithTag("all_canvases").GetComponent<CanvasManager>().planningPop();
                }
                else
                {
                    GameObject.FindGameObjectWithTag("all_canvases").GetComponent<CanvasManager>().planningIncompletePop();
                }
    ...
    public void writeScene()
    {
        ...
        GameObject.FindGameObjectWithTag("all_canvases").GetComponent<CanvasManager>().slideInputPop();
    }

    public void finishWriteScene()
    {
        getSelectedData().setSceneInfo(GameObject.Find("InputSlideText").GetComponent<InputField>().text);
        GameObject.FindGameObjectWithTag("all_canvases").GetComponent<CanvasManager>().slideInputHide();
    }
    ...
    public void toPlayScreenButton()
    {
        if (playReady)
        {
            GameObject.FindGameObjectWithTag("all_canvases").GetComponent<CanvasManager>().toPlay();
            setTitle();
        }
        else
        {
            GameObject.FindGameObjectWithTag("all_canvases").GetComponent<CanvasManager>().playStoryPop();

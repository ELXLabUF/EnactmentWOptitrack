...
public class SlideArrayNoChara : MonoBehaviour
{
    ...
    public void addSlideAlone()
    {
        if (slideBase != null && currentListID < 5)
        {
            ...
            newSlide.GetComponent<SlideSelectSlideNoChara>().assignID(currentListID); //assign it's id
            ...
            newSlide.GetComponent<SlideSelectSlideNoChara>().newPosition(buffer); //set slides position
    ...
    public void addSlide()
    {
        ...
        if (GameObject.Find("SlideSectionsVideo").GetComponent<SlideNumberingNoChara>().getCondition() == 2 || GameObject.Find("SlideSectionsVideo").GetComponent<SlideNumberingNoChara>().getTotal() == 0 || GameObject.Find("SlideSectionsVideo").GetComponent<SlideNumberingNoChara>().getCondition() == 1)
        {
            if (slideBase != null && currentListID < 5)
            {
        ...
        else if (GameObject.Find("SlideSectionsVideoVideo").GetComponent<SlideNumberingNoChara>().getAddReady() == true)
        {
            ...
            {
                case "green":
                    GameObject.Find("SlideSectionsVideo").GetComponent<SlideNumberingNoChara>().setAddWhich(0);
                    break;
                case "yellow":
                    GameObject.Find("SlideSectionsVideo").GetComponent<SlideNumberingNoChara>().setAddWhich(1);
                    break;
                case "red":
                    GameObject.Find("SlideSectionsVideo").GetComponent<SlideNumberingNoChara>().setAddWhich(2);
                    break;
                default:
                    break;
    ...
    {
        SlideSelectSlideNoChara[] slides = GetComponentsInChildren<SlideSelectSlideNoChara>(); //get all slides

        for (int i = deletedID + 1; i < currentListID; i++)
        {
            slides[i].GetComponent<SlideSelectSlideNoChara>().moveLeft(); //subtracts one from slide's id
            slides[i].GetComponent<SlideSelectSlideNoChara>().newPosition(buffer); //reset the slide's position   
        }
        currentListID--;
    }
    ...
    */
    ...
    {
        SlideSelectSlideNoChara[] slides = GetComponentsInChildren<SlideSelectSlideNoChara>(); //get all slides in array
        ...
        for (int i = 0; i < currentListID; i++)
        {
            slides[newIDs[i]].GetComponent<SlideSelectSlideNoChara>().assignID(slideIDs[i]);
            slides[newIDs[i]].GetComponent<SlideSelectSlideNoChara>().newPosition(buffer);
        }


        /*For the function in SlideNumberingNoChara to work that sets all the slide number values, the slides have to be in proper order in the heirarchy*/
        for (int i = 0; i < currentListID; i++)
        {
